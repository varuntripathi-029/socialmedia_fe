...
import { MapPin, Calendar, Users, Flame, CalendarDays } from 'lucide-react';
    ...
    return (
        <div className="space-y-6">
            ...
            <Tabs defaultValue="all" className="w-full">
                <TabsList className="grid w-full max-w-[400px] grid-cols-2 rounded-full p-1 bg-muted">
                    <TabsTrigger value="all" className="flex items-center gap-2 rounded-full py-1.5 font-semibold text-sm">
                        <CalendarDays className="h-4 w-4" />
                        All Events
                    </TabsTrigger>
                    <TabsTrigger value="trending" className="flex items-center gap-2 rounded-full py-1.5 font-semibold text-sm">
                ...
                <TabsContent value="all" className="mt-6">
                    {renderEventGrid(events, "No events yet. Be the first to create one!")}
                </TabsContent>

                <TabsContent value="trending" className="mt-6">
                    {renderEventGrid(trendingEvents, "No trending events at the moment. Join an event to make it trend!")}
                </TabsContent>
            </Tabs>
        </div>
    );
